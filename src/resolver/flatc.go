...
func ResolveFlatc(flagPath string) (string, error) {
	...
	if envPath := os.Getenv("XPLATTERGY_FLATC_PATH"); envPath != "" {
		if _, err := os.Stat(envPath); err != nil {
			return "", fmt.Errorf("flatc not found at XPLATTERGY_FLATC_PATH: %s", envPath)
		}
		return envPath, nil
	}
	...
	if err != nil {
		return "", fmt.Errorf("flatc not found in PATH; set --flatc flag or XPLATTERGY_FLATC_PATH environment variable")
	}
	return path, nil
}
