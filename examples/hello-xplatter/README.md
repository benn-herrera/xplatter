...
The xplatter codegen tool parses these `.fbs` schemas and generates C type definitions
...
- xplatter binary built (run `make` from the project root)
- C compiler (cc)
...
Each example runs `xplatter generate` to produce scaffolding in a `generated/`
