// Generated by xplattergy — do not edit.

import Foundation
import CHelloXplattergy

public enum HelloErrorCode: Int32, Error {
    case ok = 0
    case invalidArgument = 1
    case outOfMemory = 2
    case notFound = 3
    case internalError = 4
}

public final class Greeter {
    let handle: OpaquePointer

    init(handle: OpaquePointer) {
        self.handle = handle
    }

    deinit {
        hello_xplattergy_lifecycle_destroy_greeter(handle)
    }

    public static func createGreeter() throws -> Greeter {
        var result: OpaquePointer?
        let code = hello_xplattergy_lifecycle_create_greeter(&result)
        guard code == 0, let ptr = result else {
            throw HelloErrorCode(rawValue: code) ?? HelloErrorCode.internalError
        }
        return Greeter(handle: ptr)
    }

    public func sayHello(name: String) throws -> Hello_Greeting {
        var result: Hello_Greeting = Hello_Greeting()
        return try name.withCString { namePtr in
            let code = hello_xplattergy_greeter_say_hello(handle, namePtr, &result)
            guard code == 0 else {
                throw HelloErrorCode(rawValue: code) ?? HelloErrorCode.internalError
            }
            return result
        }
    }

}

